...
public class StudentHomeDecorator extends HomePageDecorator {
    ...
    public StudentHomeDecorator(HomePageComponent component) {
        super(component);
    }
    ...
    public void decorateContent(VBox contentArea) {
        ...
        Button btnBookTutor = createStyledButton("Book Tutor", 280, 55);
        btnBookTutor.setOnAction(event -> Navigator.getInstance().goTo(event, "SearchTutor.fxml"));

        Button btnCourseDiscovery = createStyledButton("Course Discovery", 280, 55);
        btnCourseDiscovery.setOnAction(event -> Navigator.getInstance().goTo(event, "SearchCourse.fxml"));

        contentArea.getChildren().addAll(btnBookTutor, btnCourseDiscovery);
    }
